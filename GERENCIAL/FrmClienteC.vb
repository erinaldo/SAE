...
Public Class FrmClienteC
    ...
    Public Sub cargarclientes()
        Try
            ...
            Next
            FrmSelCliente.lbform.Text = "Anal_cart"
            FrmSelCliente.ShowDialog()
        Catch ex As Exception
        End Try
    End Sub

    Private Sub FrmClienteC_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        With gmes
    ...
    Private Sub cmdBuscar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdBuscar.Click
        ...
        '  "SELECT doc FROM `cobdpen`  where nitc ='" & txtnitc.Text & "'  AND " _
        ...
        If Strings.Left(f1, 4) <> Strings.Left(PerActual, 4) Then
            For i = 1 To CInt(Strings.Mid(f2, 4, 2))
                ...
                If CInt(Strings.Mid(f1, 4, 2)) = CInt(Strings.Mid(f2, 4, 2)) Then
                    sql = " SELECT f.fecha, f.doc FROM facturas" & p & " f  WHERE f.nitc = '" & txtnitc.Text & "' "
                ElseIf p <> CInt(Strings.Mid(f2, 4, 2)) Then
                    sql = sql & " SELECT f.fecha, f.doc  FROM facturas" & p & " f  WHERE f.nitc = '" & txtnitc.Text & "' UNION "
                Else
                    sql = sql & " SELECT f.fecha, f.doc FROM facturas" & p & " f  WHERE f.nitc = '" & txtnitc.Text & "' "
                End If
            Next
        Else
        ...
        If tabla2.Rows.Count = 0 Then
            MsgBox("En este rango de fechas el cliente no ha realizado compras, ", MsgBoxStyle.Information)
            Exit Sub
        Else
            docu = "("
    ...
    Private Sub facturas()
        ...
        If Strings.Left(f1, 4) <> Strings.Left(PerActual, 4) Then
            For i = 1 To CInt(Strings.Mid(f2, 4, 2))
                ...
                End If
                If CInt(Strings.Mid(f1, 4, 2)) = CInt(Strings.Mid(f2, 4, 2)) Then
                    sql = " SELECT f.fecha, f.doc, f.subtotal, f.descuento, f.total, v.nombre FROM facturas" & p & " f LEFT JOIN vendedores v ON f.nitv= v.nitv WHERE f.nitc = '" & txtnitc.Text & "' "
                ElseIf p <> CInt(Strings.Mid(f2, 4, 2)) Then
                    sql = sql & " SELECT f.fecha, f.doc, f.subtotal, f.descuento, f.total, v.nombre  FROM facturas" & p & " f LEFT JOIN vendedores v ON f.nitv= v.nitv WHERE f.nitc = '" & txtnitc.Text & "' UNION "
                Else
                    sql = sql & " SELECT f.fecha, f.doc, f.subtotal, f.descuento, f.total,  v.nombre  FROM facturas" & p & " f LEFT JOIN vendedores v ON f.nitv= v.nitv WHERE f.nitc = '" & txtnitc.Text & "' "
                End If
            Next
        Else
            For i = CInt(Strings.Mid(f1, 4, 2)) To CInt(Strings.Mid(f2, 4, 2))
        ...
        For j = 0 To items - 1
            ...
            Else
                ...
                tt = tt + gventasF.Item(4, j).Value
            End If
            gventasF.Item(5, j).Value = tb.Rows(j).Item("nombre")
        Next
        ...
        If Strings.Left(f1, 4) <> Strings.Left(PerActual, 4) Then
            For k = 1 To CInt(Strings.Mid(f2, 4, 2))
                ...
                End If
                If CInt(Strings.Mid(f1, 4, 2)) = CInt(Strings.Mid(f2, 4, 2)) Then
                    sql2 = "SELECT '" & p & "' as mes, sum(c.sub) as subt, sum(c.des) as dsc, sum(c.tot) as tt, IF( IFNULL(SUM( c.sub ),'Y') <>'Y', 'N', 'Y' )  as n  FROM( SELECT f.doc, if (left(f.doc,2)='" & doc_aj & "',concat('-',f.subtotal), f.subtotal) as sub, if (left(f.doc,2)='" & doc_aj & "',concat('-',f.descuento), f.descuento) as des, if (left(f.doc,2)='" & doc_aj & "',concat('-',f.total), f.total) as tot FROM facturas" & p & " f  WHERE f.nitc = '" & txtnitc.Text & "' ) as c "
                ElseIf p <> CInt(Strings.Mid(f2, 4, 2)) Then
                    sql2 = sql2 & " SELECT '" & p & "' as mes, sum(c.sub) as subt, sum(c.des) as dsc, sum(c.tot) as tt, IF( IFNULL(SUM( c.sub ),'Y') <>'Y', 'N', 'Y' )  as n  FROM( SELECT f.doc, if (left(f.doc,2)='" & doc_aj & "',concat('-',f.subtotal), f.subtotal) as sub, if (left(f.doc,2)='" & doc_aj & "',concat('-',f.descuento), f.descuento) as des, if (left(f.doc,2)='" & doc_aj & "',concat('-',f.total), f.total) as tot FROM facturas" & p & " f  WHERE f.nitc = '" & txtnitc.Text & "' ) as c UNION "
                Else
                    sql2 = sql2 & "SELECT '" & p & "' as mes, sum(c.sub) as subt, sum(c.des) as dsc, sum(c.tot) as tt, IF( IFNULL(SUM( c.sub ),'Y') <>'Y', 'N', 'Y' )  as n  FROM( SELECT f.doc, if (left(f.doc,2)='" & doc_aj & "',concat('-',f.subtotal), f.subtotal) as sub, if (left(f.doc,2)='" & doc_aj & "',concat('-',f.descuento), f.descuento) as des, if (left(f.doc,2)='" & doc_aj & "',concat('-',f.total), f.total) as tot FROM facturas" & p & " f  WHERE f.nitc = '" & txtnitc.Text & "' ) as c "
                End If
            Next
        Else
    ...
    Private Sub cartera()
        ...
        Dim tt As Double = 0
        Dim tb As New DataTable
        myCommand.CommandText = "SELECT doc, fecha, vmto, total, (total-pagado) as saldo FROM `cobdpen` WHERE nitc = '" & txtnitc.Text & "' ORDER BY fecha"
        myAdapter.SelectCommand = myCommand
        myAdapter.Fill(tb)
        Refresh()
        gcartera.Rows.Clear()
        ...
        txt_ttcart.Text = Moneda(tt)


        sql = "SELECT IFNULL(SUM(c.retiva),0) as s3, IFNULL(sum(c.retica),0) as s6, IFNULL(sum(c.pagado),0) as s9, IFNULL(sum(c.vpos),0) as s1, IFNULL(sum(c.tasa),0) as s12 FROM( "
        sql = sql & " SELECT c.nitc, c.nomnit, t.telefono AS nitcod, c.doc, CAST( c.fecha AS CHAR( 20 ) ) AS fecha, " _
    & " CAST( (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) AS CHAR( 20 ) ) as nitv, " _
    & " if(left(c.doc,2)='" & doc_aj & "',IF ( (SELECT DATEDIFF( NOW( ) ,  (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) ) ) < 30, -(c.total - c.pagado), 0),IF ( (SELECT DATEDIFF( NOW( ) ,  (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) ) ) < 30, (c.total - c.pagado), 0)) AS retiva,  " _
    & " if(left(c.doc,2)='" & doc_aj & "', IF ((SELECT DATEDIFF( NOW( ) ,  (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) ) ) > 31 AND (SELECT DATEDIFF( NOW( ) ,  (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) ) ) < 60 , -(c.total - c.pagado), 0 ),IF ((SELECT DATEDIFF( NOW( ) ,  (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) ) ) > 31 AND (SELECT DATEDIFF( NOW( ) ,  (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) ) ) < 60 , (c.total - c.pagado), 0 )) AS retica, " _
    & " if(left(c.doc,2)='" & doc_aj & "', IF ( (SELECT DATEDIFF( NOW( ) ,  (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) ) ) > 61 AND (SELECT DATEDIFF( NOW( ) ,  (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) ) ) < 90 , -(c.total - c.pagado), 0 ),IF ( (SELECT DATEDIFF( NOW( ) ,  (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) ) ) > 61 AND (SELECT DATEDIFF( NOW( ) ,  (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) ) ) < 90 , (c.total - c.pagado), 0 )) AS pagado, " _
    & " if(left(c.doc,2)='" & doc_aj & "', IF ( (SELECT DATEDIFF( NOW( ) ,  (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) ) ) > 91 AND (SELECT DATEDIFF( NOW( ) ,  (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) ) ) < 120 , -(c.total - c.pagado), 0 ),IF ( (SELECT DATEDIFF( NOW( ) ,  (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) ) ) > 91 AND (SELECT DATEDIFF( NOW( ) ,  (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) ) ) < 120 , (c.total - c.pagado), 0 )) AS vpos, " _
    & " if(left(c.doc,2)='" & doc_aj & "', IF ( (SELECT DATEDIFF( NOW( ) ,  (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) ) ) > 121, -(c.total - c.pagado), 0 ),IF ( (SELECT DATEDIFF( NOW( ) ,  (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) ) ) > 121, (c.total - c.pagado), 0 )) AS tasa, " _
    & " (total - pagado) AS total FROM cobdpen c, terceros t " _
    & " WHERE c.nitc = '" & txtnitc.Text & "' AND  t.nit= '" & txtnitc.Text & "' AND c.total > c.pagado AND (SELECT ADDDATE( c.fecha, INTERVAL c.vmto DAY) ) BETWEEN '" & fe1 & "' AND '" & fe2 & "'  "
        sql = sql & ") as c "
        ...
        txt3.Text = Moneda(tb_c.Rows(0).Item(0))
        txt6.Text = Moneda(tb_c.Rows(0).Item(1))
        txt9.Text = Moneda(tb_c.Rows(0).Item(2))
        txt120.Text = Moneda(tb_c.Rows(0).Item(3))
        txt121.Text = Moneda(tb_c.Rows(0).Item(4))
    ...
    Private Sub productos(ByVal docu As String)
        ...
        If Strings.Left(f1, 4) <> Strings.Left(PerActual, 4) Then
            For i = 1 To CInt(Strings.Mid(f2, 4, 2))
                ...
                End If
                If CInt(Strings.Mid(f1, 4, 2)) = CInt(Strings.Mid(f2, 4, 2)) Then
                    sql = "SELECT doc, item, codart, nomart, cantidad, vtotal, '" & p & "' as mes FROM detafac" & p & " where doc IN  " & docu & " "
                ElseIf p <> CInt(Strings.Mid(f2, 4, 2)) Then
                    sql = sql & "SELECT doc, item, codart, nomart, cantidad, vtotal, '" & p & "' as mes FROM detafac" & p & " where doc IN  " & docu & " UNION "
                Else
                    sql = sql & "SELECT doc, item, codart, nomart, cantidad, vtotal, '" & p & "' as mes FROM detafac" & p & " where doc IN  " & docu & " "
                End If
            Next
        Else
            For i = CInt(Strings.Mid(f1, 4, 2)) To CInt(Strings.Mid(f2, 4, 2))
        ...
        For j = 0 To t_doc.Rows.Count - 1
            ...
            gproductos.Item(1, j).Value = t_doc.Rows(j).Item("codart")
            gproductos.Item(2, j).Value = t_doc.Rows(j).Item("nomart")
        ...
        If Strings.Left(f1, 4) <> Strings.Left(PerActual, 4) Then
            ...
            For i = 1 To CInt(Strings.Mid(f2, 4, 2))
                ...
                If 1 = CInt(Strings.Mid(f2, 4, 2)) Then
                    sql = "SELECT doc, fecha,doc_afec, descrip, concat(dia,'/',periodo) as f2, abonado FROM ot_cpc" & p & "  where doc_afec IN  " & docu & " and doc_aj=''"
                ElseIf p <> CInt(Strings.Mid(f2, 4, 2)) Then
                    sql = sql & "SELECT doc, fecha,doc_afec, descrip, concat(dia,'/',periodo) as f2, abonado FROM ot_cpc" & p & "  where doc_afec IN  " & docu & " and doc_aj='' UNION "
                Else
                    sql = sql & " SELECT doc, fecha,doc_afec, descrip, concat(dia,'/',periodo) as f2, abonado FROM ot_cpc" & p & "  where doc_afec IN  " & docu & " and doc_aj=''"
                End If
            Next
        Else
            For i = CInt(Strings.Mid(f1, 4, 2)) To CInt(Strings.Mid(f2, 4, 2))
    ...
    Private Sub seleccionardoc(ByVal mifila As Integer)
        ...
        Try
            ...
            myCommand.CommandText = "SELECT item, codart, nomart, cantidad, valor from detafac" & per & " where doc= '" & gventasF.Item(1, mifila).Value & "' ORDER BY item;"
            myAdapter.SelectCommand = myCommand
            myAdapter.Fill(tbd)
            Refresh()
            ...
            For j = 0 To tbd.Rows.Count - 1
                ...
                Frmdetafac.gdetaF.Item(1, j).Value = tbd.Rows(j).Item("codart")
                Frmdetafac.gdetaF.Item(2, j).Value = tbd.Rows(j).Item("nomart")
    ...
    Private Sub gventasF_CellDoubleClick1(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles gventasF.CellDoubleClick
        seleccionardoc(fila)
    End Sub

    Private Sub gventasF_CellEnter1(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles gventasF.CellEnter
        fila = e.RowIndex
    End Sub

    Private Sub gventasF_KeyPress1(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles gventasF.KeyPress
    ...
    Private Sub txtmes_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtmes.SelectedIndexChanged
        ...
        If gventasF.RowCount <> 0 Then
            ...
            Else
                ...
                sql = "SELECT doc, item, codart, nomart, cantidad, vtotal, '" & txtmes.Text & "' as mes FROM detafac" & txtmes.Text & " where doc IN  " & docu & " "
                sql = sql & "ORDER BY mes, doc, item"
