...
Public Class FrmCompIngresos
    ...
    Private Sub FrmComEgreso_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            myCommand.CommandText = " ALTER TABLE  `cobdpen` CHANGE  `doc`  `doc` VARCHAR( 30 ) CHARACTER SET latin1 COLLATE latin1_swedish_ci NOT NULL "
    ...
    Public Sub BuscarActual()
        If lbestado.Text = "NUEVO" Then
            ...
            myCommand.CommandText = "SELECT ci FROM parotdoc;"
            myAdapter.SelectCommand = myCommand
            myAdapter.Fill(tabla)
            If tabla.Rows.Count < 1 Then
                MsgBox("Favor no han creado tipo de documento para los comprobantes de de ingresos (del grupo CI), verifique.   ", MsgBoxStyle.Information, "Verificando")
                Me.Close()
                Exit Sub
            End If
            '**********************************
            ...
            If tabla2.Rows.Count < 1 Then
                MsgBox("Favor no han creado tipo de documento para los comprobantes de ingresos (del grupo CI), verifique.   ", MsgBoxStyle.Information, "Verificando")
                Me.Close()
            Else
            ...
            If ValidarGuardar() = True Then
                ...
                If lbestado.Text = "NUEVO" Then
                    ...
                    If FrmPrincipal.cmdAuditoria.Visible = True Then
                        Guar_MovUser("CONTABILIDAD", "GUARDAR DOC Nº: " & lbdoc.Text & txtnumero.Text & "-" & lbper.Text, "", "", "")
                    End If
                    '.....
                ...
                Else
                    ...
                    If resultado = MsgBoxResult.Yes Then
                        ...
                        If Chcredito.Checked = True Then
                            If Guardarcobdpen() = 0 Then
                                MsgBox("Favor seleccione una cuenta de cuentas por cobrar para poder guardar el documento.", MsgBoxStyle.Information, "SAE Control de Datos")
                                Exit Sub
                            End If
                        End If
                        '********************************
                        ...
                        '.....
                        If FrmPrincipal.cmdAuditoria.Visible = True Then
                            Guar_MovUser("CONTABILIDAD", "MODIFICAR DOC Nº: " & lbdoc.Text & txtnumero.Text & "-" & lbper.Text, "", "", "")
                        End If
                        '.....
    ...
    Function Guardarcobdpen()
        ...
        If sw <> 0 Then
            Try
                ...
                myCommand.CommandText = "INSERT INTO cobdpen  VALUES(" _
                    & "?doc,?tipo,?num,?descrip,?tipafec,?clasaju,?nitc,?nomnit,?nitcod,?nitv,?fecha,?vmto,?concepto,?subtotal," _
        ...
        Try
            ...
            If sw = 0 Then
                ...
                For i = 0 To grilla.RowCount - 1
                    If ChAnti.Checked = True Then
                        Try
                            resultado = MsgBox("No ha seleccionado una cuenta de la " & tpuc.Replace("*", "").ToString & ", ¿Esta es su cuenta de anticipos " & grilla.Item("Cuenta", i).Value.ToString & "?", MsgBoxStyle.YesNo, "Verificando")
                            If resultado = MsgBoxResult.Yes Then
                                cta = grilla.Item("Cuenta", i).Value.ToString
                                sw = 1
                                Exit For
    ...
    Private Sub CmdPrimero_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdPrimero.Click
        Try
            ...
            Dim tabla As New DataTable
            myCommand.CommandText = "SELECT DISTINCT(doc) FROM " & mes & " WHERE grupo='CI' ORDER BY doc, item LIMIT 0, 1;"
            myAdapter.SelectCommand = myCommand
            myAdapter.Fill(tabla)
    ...
    Private Sub CmdAtras_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdAtras.Click
        Try
            ...
            If i > 0 Then
                i = i - 1
                Dim tabla As New DataTable
                myCommand.CommandText = "SELECT DISTINCT(doc) FROM " & mes & " WHERE grupo='CI' ORDER BY doc, item LIMIT " & i & ", 1;"
                myAdapter.SelectCommand = myCommand
                myAdapter.Fill(tabla)
                Refresh()
                BuscarDocumento(tabla.Rows(0).Item(0))
    ...
    Private Sub CmdSiguiente_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdSiguiente.Click
        Try
            ...
            Dim tabla, tabla2 As New DataTable
            myCommand.CommandText = "SELECT DISTINCT(doc) FROM " & mes & " WHERE grupo='CI';"
            myAdapter.SelectCommand = myCommand
            myAdapter.Fill(tabla2)
            Refresh()
    ...
    Public Sub BuscarCC()
        Try
            ...
            Else
                ...
                FrmSelCentroCostos.lbform.Text = "comproIn"
    ...
    Public Sub cargarclientes()
        Try
            FrmSelCliente.lbform.Text = "comproIn"
    ...
    Public Sub BuscarClientes(ByVal nit As String)
        ...
        If items = 0 Then
            ...
            If resultado = MsgBoxResult.Yes Then
                ...
                frmterceros.lbform.Text = "comproIn"
    ...
    Private Sub Buscarcco(ByVal ccosto As String, ByVal fila As Integer)
        Try
            If ccosto = "" Then
                FrmSelCentroCostos.lbform.Text = "comproIgG"
    ...
    Public Sub Buscarcuentas(ByVal cuenta As String, ByVal fila As Integer)
        Try
            If cuenta = "" Then
                FrmCuentas.lbform.Text = "comproIn"
    ...
    Private Sub AuditoriaCI()
        If FrmPrincipal.cmdAuditoria.Visible = True Then
            ...
            '                    Vali_cuent_Odoc("caj", lbdoc.Text & "-" & lbnomdoc.Text, grilla.Item("Cuenta", i).Value.ToString, lbdoc.Text & txtnumero.Text & "-" & lbper.Text, "")
            '                ElseIf Chcredito.Checked = True Then
            '                    Vali_cuent_Odoc("cxp", lbdoc.Text & "-" & lbnomdoc.Text, grilla.Item("Cuenta", i).Value.ToString, lbdoc.Text & txtnumero.Text & "-" & lbper.Text, "")
            '                Else
            '                    Vali_cuent_Odoc("ban", lbdoc.Text & "-" & lbnomdoc.Text, grilla.Item("Cuenta", i).Value.ToString, lbdoc.Text & txtnumero.Text & "-" & lbper.Text, "")
            '                End If
            '            End If
    ...
    Public Sub Guardar(ByVal fila As Integer)
        ...
        myCommand.Parameters.AddWithValue("?grupo", "CI")
        myCommand.Parameters.AddWithValue("?tipodoc", lbdoc.Text)
    ...
    Function Eliminar()
        ...
        myCommand.CommandText = "SELECT pagado FROM cobdpen WHERE doc='" & PerActual & "-" & lbdoc.Text & txtnumero.Text & "' ;"
        myAdapter.SelectCommand = myCommand
        myAdapter.Fill(tc)
    ...
    Public Sub GenerarPDF()
        ...
        Try
            ...
            cb.ShowTextAligned(PdfContentByte.ALIGN_LEFT, lbnomdoc.Text, 388, 820, 0)
            cb.EndText()
            cb.BeginText()
            ...
            '********** ANTICIPO *****************
            If ChAnti.Checked = True Then
                cb.ShowTextAligned(PdfContentByte.ALIGN_LEFT, "X", 573, 705, 0)
            End If
            '*******FORMA DE PAGO*****************
            If Chcredito.Checked = True Then
                cb.ShowTextAligned(PdfContentByte.ALIGN_LEFT, "X", 580, 589, 0)
    ...
    Private Sub ColocarLogo()
        Try
            ...
            img.SetAbsolutePosition(10, 799)
    ...
    Private Sub CmdCons_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdCons.Click
        Try
            ...
            FrmSelOtdoc.lbform.Text = "ComIngre"
