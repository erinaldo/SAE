...
Public Class FrmLibroDiario
    ...
    Private Sub txtdia_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtdia.LostFocus
        ...
        End Try
    End Sub
    Private Sub nivel_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs)
    ...
    Public Sub Generar()
        ...
        If txtdoc2.Text <> "" Then 'POR TIPO DE DOCUMENTO
            sql = "SELECT * FROM documentos" & PerActual(0) & PerActual(1) & " WHERE (dia='" & txtdia.Text & "' OR dia='" & Val(txtdia.Text) & "') AND tipodoc='" & Trim(txtdoc.Text) & "' ORDER BY tipodoc,doc,item;"
        Else ' TODOS LOS DOCUEMTOS
            sql = "SELECT * FROM documentos" & PerActual(0) & PerActual(1) & " WHERE dia='" & txtdia.Text & "' OR dia='" & Val(txtdia.Text) & "' ORDER BY tipodoc,doc,item;"
        End If
        Dim tabla As New DataTable
    ...
    Public Sub GenerarPDF()
        ...
        Try
            ...
            For i = 0 To grilla.RowCount - 1
                k = k - 19
                If i = grilla.RowCount - 1 Then tope = 85
                If k < tope Then
                    k = 760
                    cb.EndText()
                    oDoc.NewPage()
    ...
    Public Sub ColocarImg(ByVal i As Integer)
        Try
            ...
            If i = 1 Then
                img = iTextSharp.text.Image.GetInstance(My.Application.Info.DirectoryPath & "\Reportes\CD\LD1.jpg")
            ...
            Else
                img = iTextSharp.text.Image.GetInstance(My.Application.Info.DirectoryPath & "\Reportes\CD\LD3.jpg")
                img.SetAbsolutePosition(0, k)
                img.ScaleToFit(598, 100)
            End If
