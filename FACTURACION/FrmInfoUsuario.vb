...
Public Class FrmInfoUsuario
    ...
    Private Sub txttip_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txttip.LostFocus
        ...
        myCommand.CommandText = "SELECT * FROM sae.usuarios WHERE login='" & txttip.Text & "';"
        myAdapter.SelectCommand = myCommand
        myAdapter.Fill(t)
        If t.Rows.Count = 0 Then
            txtnom.Clear()
            Try
                FrmSelUsuario.lbform.Text = "Fusu"
                FrmSelUsuario.ShowDialog()
            Catch ex As Exception
            End Try
        Else
            txtnom.Text = t.Rows(0).Item("nombres").ToString() & " " & t.Rows(0).Item("apellidos").ToString()
        End If
    ...
    Private Sub cmdpantalla_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdpantalla.Click
        ...
        Try
            If FrmPrincipal.cmdAuditoria.Visible = True Then
                Guar_MovUser("FACTURACION", "VISUALIZAR INFORME POR USUARIO ", "", "", "")
            End If
            ...
            If d1.Checked = True Then
                For i = Val(cbini.Text) To Val(cbfin.Text)
                    ...
                    If cbini.Text = cbfin.Text Then
                        sql = " SELECT f.doc , UCASE(f.usuario) as nitv, CAST(CONCAT(s.nombres,' ', s.apellidos) AS CHAR(100)) AS ciu_ent, t.nombre As cc ,  df.nomart AS observ , " _
                           ...
                           & " FROM detafac" & p & " df, sae.usuarios s, facturas" & p & " f ,terceros t " _
                           & " WHERE t.nit = f.nitc and f.doc = df.doc  AND  s.login = f.usuario " & n & " AND right(f.fecha,2) BETWEEN '" & txtdi1.Text & "' AND  '" & txtdi2.Text & "'"
                    Else
                        If p = cbini.Text Then
                            ...
                            & " (((df.vtotal/(1+(df.iva_d/100))-(df.vtotal/(1+(df.iva_d/100)) *(df.por_des/100)) )) - ( ((df.vtotal/(1+(df.iva_d/100))-(df.vtotal/(1+(df.iva_d/100)) *(df.por_des/100)) )) * (f.por_desc/100))) * (1+(df.iva_d/100)) AS subtotal " _
                           & " FROM detafac" & p & " df, sae.usuarios s, facturas" & p & " f , terceros t " _
                           & "  WHERE t.nit = f.nitc  and (f.doc = df.doc) AND s.login = f.usuario " & n & " AND right(f.fecha,2) >= '" & txtdi1.Text & "'"
                        ElseIf p <> cbini.Text And p <> cbfin.Text Then
                            sql = sql & " UNION  SELECT f.doc , UCASE(f.usuario) as nitv, CAST(CONCAT(s.nombres,' ', s.apellidos) AS CHAR(100)) AS ciu_ent, t.nombre As cc ,  df.nomart AS observ , " _
                           & " CAST( (CONCAT( RIGHT( f . fecha , 2 ) ,  '/', MID( f . fecha ,  6, 2 ) ,  '/', LEFT( f . fecha , 4 ) ) ) AS CHAR( 20 ) ) AS fecha_o, " _
                            & " (((df.vtotal/(1+(df.iva_d/100))-(df.vtotal/(1+(df.iva_d/100)) *(df.por_des/100)) )) - ( ((df.vtotal/(1+(df.iva_d/100))-(df.vtotal/(1+(df.iva_d/100)) *(df.por_des/100)) )) * (f.por_desc/100))) * (1+(df.iva_d/100)) AS subtotal " _
                           & " FROM detafac" & p & " df, sae.usuarios s, facturas" & p & " f,terceros t " _
                           & "  WHERE t.nit = f.nitc and (f.doc = df.doc)  AND s.login = f.usuario " & n & ""
                ...
                CrReport.Load(My.Application.Info.DirectoryPath & "\Reportes\ReportFUsu.rpt")
                ...
                Try
                    ...
                    Prtt.Name = "tit"
                    Prtt.CurrentValues.AddValue(" USUARIOS")
            ...
            Else
                ...
                sql = "SELECT  c.nitv, c.ciu_ent, SUM(c.subtotal) as subtotal FROM ("
                For i = Val(cbini.Text) To Val(cbfin.Text)
                    If i < 10 Then
                        p = "0" & i
                    Else
                    ...
                    If cbini.Text = cbfin.Text Then
                        sql = sql & " SELECT f.doc , UCASE(f.usuario) as nitv, CAST(CONCAT(s.nombres,' ', s.apellidos) AS CHAR(100)) AS ciu_ent, t.nombre As cc ,  df.nomart AS observ , " _
                            & " CAST( (CONCAT( RIGHT( f . fecha , 2 ) ,  '/', MID( f . fecha ,  6, 2 ) ,  '/', LEFT( f . fecha , 4 ) ) ) AS CHAR( 20 ) ) AS fecha_o, " _
                            & "IF(LEFT(f.doc,2)='AF',  " _
                            & " -1 * (((df.vtotal/(1+(df.iva_d/100))-(df.vtotal/(1+(df.iva_d/100)) *(df.por_des/100)) )) - ( ((df.vtotal/(1+(df.iva_d/100))-(df.vtotal/(1+(df.iva_d/100)) *(df.por_des/100)) )) * (f.por_desc/100))) * (1+(df.iva_d/100)), " _
                ...
                CrReport.Load(My.Application.Info.DirectoryPath & "\Reportes\ReportFUsu2.rpt")
                CrReport.SetDataSource(tabla)
