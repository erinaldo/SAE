Public Class FrmBuscarCodBar
    ...
    Private Sub txtcod_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtcod.KeyPress
        ...
        End If
    End Sub
    Public Sub BuscarCod()
        If lbsalir.Text = "SI" Then
            'NO HACER NADA PARA SALIR
        ElseIf Trim(txtcod.Text) = "" Then
            MsgBox("Digete codigo de barras.  ", MsgBoxStyle.Information, "SAE Control")
            txtcod.Focus()
        ...
        Else
            Try
                ...
                End Select
                myCommand.CommandText = "SELECT a.*, (SELECT IF(margmin='S',CAST(margen AS SIGNED),0) FROM parafacts WHERE factura='RAPIDA') AS margin FROM articulos a WHERE codbar='" & Trim(txtcod.Text) & "' AND nivel='Articulo' And estado='Activo' ORDER BY codart;"
                myAdapter.SelectCommand = myCommand
                myAdapter.Fill(tabla)
                Refresh()
                items = tabla.Rows.Count
                If items < 1 Then
                    MsgBox("El Codigo de Barras(" & Trim(txtcod.Text) & ") no se encuentra en los registros, verifique.  ", MsgBoxStyle.Information, "SAE")
                    txtcod.Text = ""
                    txtcod.Focus()
                Else
                    Select Case lbform.Text
                        Case "itemEst"
                            ...
                            Try
                                FrmItemsEst.gitems.Item("disponible", fila).Value = FrmItemsEst.gitems.Item("cant", fila).Value
                            Catch ex As Exception
                                'FrmItemsEst.gitems.Item("disponible", fd).Value = "0"
                            End Try
                            FrmItemsEst.gitems.Item("precio2", fila).Value = FrmItemsEst.gitems.Item("precio", fila).Value
                            txtcod.Text = ""
                            FrmItemsEst.gitems.Item("codcom", fila).Value = ccm
                            FrmItemsEst.BuscarArticulos(fila, tabla.Rows(0).Item("codart"), "I")
                            Me.Close()
                        Case "items"
                            ...
                            Try
                                FrmItems.gitems.Item("disponible", fila).Value = FrmItems.gitems.Item("cant", fila).Value
                            Catch ex As Exception
                                'FrmItems.gitems.Item("disponible", fd).Value = "0"
                            End Try
                            FrmItems.gitems.Item("precio2", fila).Value = FrmItems.gitems.Item("precio", fila).Value
                            txtcod.Text = ""
                            FrmItems.gitems.Item("cc", fila).Value = ""
                            FrmItems.BuscarArticulos(fila, tabla.Rows(0).Item("codart"), "I")
                            Me.Close()
                        Case "itemsC"
    ...
    Private Sub txtcod_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtcod.LostFocus
        BuscarCod()
    End Sub
    Private Sub FrmBuscarCodBar_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        lbsalir.Text = "SI"
        txtcod.Text = ""
        txtcod.Focus()
    End Sub

    Private Sub FrmBuscarCodBar_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            lbsalir.Text = "NO"
            txtcod.Text = ""
            txtcod.Focus()
