...
Public Class FrmInfoServicios

    Private Sub FrmInfoServicios_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Public Sub mostrar_pdf()
        ...
        cadena = "INFORME DE FACTURACION POR SERVICIOS"
        cb.ShowTextAligned(50, "-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- ", 0, 750, 0)
        cb.ShowTextAligned(50, cadena, 300, 740, 0)
        ...
        For i = CDbl(cbini.Text) To CDbl(cbfin.Text)
            ...
            If consulta = "" Then
                ...
                If d1.Checked = True Then
                    consulta = "SELECT " & t2 & ".codart," & t2 & ".nomart," & t1 & ".tipodoc," & t1 & ".num,DATE_FORMAT(" & t1 & ".fecha,'%d/%m/%y') as fecha," & t3 & ".terceros.nombre," & t3 & ".terceros.apellidos," & t2 & ".cantidad," & t2 & ".valor as subtotal," & t1 & ".descuento," & t1 & ".iva," & t2 & ".vtotal as total," & t2 & ".iva_d," & t2 & ".valor  FROM " & t3 & ".terceros INNER JOIN (" & t1 & " INNER JOIN " & t2 & " ON " & t2 & ".doc=" & t1 & ".doc) ON " & t1 & ".nitc=" & t3 & ".terceros.nit where " & t1 & ".doc_de<>'' "
                    cad3 = ""
                Else
                    consulta = "SELECT " & t2 & ".codart," & t2 & ".nomart," & "SUM(" & t2 & ".cantidad) as cantidad,SUM(" & t1 & ".descuento) as descuento,SUM(" & t1 & ".iva) as iva,SUM(" & t2 & ".vtotal) as total," & t2 & ".iva_d," & t2 & ".valor FROM " & t3 & ".terceros INNER JOIN (" & t1 & " INNER JOIN " & t2 & " ON " & t2 & ".doc=" & t1 & ".doc) ON " & t1 & ".nitc=" & t3 & ".terceros.nit where " & t1 & ".doc_de<>'' "
            ...
            Else
                If d1.Checked = True Then
                    ...
                    cad3 = ""
                Else
                    consulta2 = "SELECT " & t2 & ".codart," & t2 & ".nomart," & "SUM(" & t2 & ".cantidad) as cantidad,SUM(" & t1 & ".descuento) as descuento,SUM(" & t1 & ".iva) as iva,SUM(" & t1 & ".total) as total," & t2 & ".iva_d," & t2 & ".valor  FROM " & t3 & ".terceros INNER JOIN (" & t1 & " INNER JOIN " & t2 & " ON " & t2 & ".doc=" & t1 & ".doc) ON " & t1 & ".nitc=" & t3 & ".terceros.nit where " & t1 & ".doc_de<>'' "
                    cad3 = " GROUP BY codart,nomart "
                End If
    ...
    Private Sub cmdpantalla_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdpantalla.Click
        ...
        Try
            ...
            If FrmPrincipal.cmdAuditoria.Visible = True Then
                Guar_MovUser("FACTURACION", "VISUALIZAR INFORME POR SERVICIO ", "", "", "")
            End If
            ...
            tit = "INFORME DE FACTURACION POR SERVICIOS"
            ...
            If d1.Checked = True Then
                For i = Val(cbini.Text) To Val(cbfin.Text)
                    ...
                    If cbini.Text = cbfin.Text Then
                        sql = " SELECT '' as concep, concat(f.doc,' - ',df.nomart) AS doc, df.nomart AS nomart, df.codart ,( TRIM(CONCAT(t.nombre,' ',t.apellidos))) AS cta_cos , " _
                           ...
                           & " ((df.valor-(df.valor*(df.por_des/100))) - (( ((df.valor-(df.valor*(df.por_des/100))) / ( 1 + ( df.iva_d /100 ) ))* f.por_desc /100) ) - (((df.valor-(df.valor*(df.por_des/100)))-((df.valor-(df.valor*(df.por_des/100)))/(1+(df.iva_d /100))))) - ((((df.valor-(df.valor*(df.por_des/100)))/(1+(df.iva_d /100))) * f.por_ret_f/100))) As valor , " _
                             ...
                             & " FROM  detafac" & p & " df, facturas" & p & " f , terceros t WHERE f.nitc = t.nit " _
                         & " AND (f.doc = df.doc) " & c & "  AND df.tipo_it = 'S' AND right(f.fecha,2) BETWEEN '" & txtdi1.Text & "' AND  '" & txtdi2.Text & "' "
                    Else
                        If p = cbini.Text Then
                            sql = " SELECT '' as concep,concat(f.doc,' - ',df.nomart) AS doc, df.nomart AS nomart, df.codart ,( TRIM(CONCAT(t.nombre,' ',t.apellidos))) AS cta_cos , " _
                             ...
                             & " (((df.vtotal/(1+(df.iva_d/100))-(df.vtotal/(1+(df.iva_d/100)) *(df.por_des/100)) )) - ( ((df.vtotal/(1+(df.iva_d/100))-(df.vtotal/(1+(df.iva_d/100)) *(df.por_des/100)) )) * (f.por_desc/100))) * (1+(df.iva_d/100)) AS vtotal " _
                             & " FROM  detafac" & p & " df, facturas" & p & " f , terceros t WHERE f.nitc = t.nit " _
                         & " AND (f.doc = df.doc) " & c & "  AND df.tipo_it = 'S' AND right(f.fecha,2) >= '" & txtdi1.Text & "' "
                        ElseIf p <> cbini.Text And p <> cbfin.Text Then
                            sql = sql & " UNION  SELECT '' as concep,concat(f.doc,' - ',df.nomart) AS doc, df.nomart AS nomart, df.codart ,( TRIM(CONCAT(t.nombre,' ',t.apellidos))) AS cta_cos , " _
                              & " CAST( f.fecha AS CHAR( 20 ) ) AS cta_inv  , df.cantidad AS item,  " _
                              & " ((df.valor-(df.valor*(df.por_des/100))) - (( ((df.valor-(df.valor*(df.por_des/100))) / ( 1 + ( df.iva_d /100 ) ))* f.por_desc /100) ) - (((df.valor-(df.valor*(df.por_des/100)))-((df.valor-(df.valor*(df.por_des/100)))/(1+(df.iva_d /100))))) - ((((df.valor-(df.valor*(df.por_des/100)))/(1+(df.iva_d /100))) * f.por_ret_f/100))) As valor , " _
                             & "(( (((df.vtotal-(df.vtotal*(df.por_des/100)))-((df.vtotal-(df.vtotal*(df.por_des/100)))*(df.por_des/100))) / ( 1 + ( df.iva_d /100 ) ))* f.por_desc /100)) AS cantidad, " _
                             & " (((df.vtotal-(df.vtotal*(df.por_des/100))) - (( ((df.vtotal-(df.vtotal*(df.por_des/100))) / ( 1 + ( df.iva_d /100 ) ))* f.por_desc /100)) - (((df.vtotal-(df.vtotal*(df.por_des/100)))-((df.vtotal-(df.vtotal*(df.por_des/100)))/(1+(df.iva_d /100))))) - ((((df.vtotal-(df.vtotal*(df.por_des/100)))/(1+(df.iva_d /100))) * f.por_ret_f/100))) * df.iva_d/100) AS costo, " _
                             & " (((df.vtotal/(1+(df.iva_d/100))-(df.vtotal/(1+(df.iva_d/100)) *(df.por_des/100)) )) - ( ((df.vtotal/(1+(df.iva_d/100))-(df.vtotal/(1+(df.iva_d/100)) *(df.por_des/100)) )) * (f.por_desc/100))) * (1+(df.iva_d/100)) AS vtotal " _
                             & " FROM  detafac" & p & " df, facturas" & p & " f , terceros t WHERE f.nitc = t.nit " _
                         & " AND(f.doc = df.doc)  " & c & " AND df.tipo_it = 'S' "
                        ElseIf p = cbfin.Text Then
                         ...
                         & " ((df.valor-(df.valor*(df.por_des/100))) - (( ((df.valor-(df.valor*(df.por_des/100))) / ( 1 + ( df.iva_d /100 ) ))* f.por_desc /100) ) - (((df.valor-(df.valor*(df.por_des/100)))-((df.valor-(df.valor*(df.por_des/100)))/(1+(df.iva_d /100))))) - ((((df.valor-(df.valor*(df.por_des/100)))/(1+(df.iva_d /100))) * f.por_ret_f/100))) As valor , " _
                             ...
                             & " (((df.vtotal/(1+(df.iva_d/100))-(df.vtotal/(1+(df.iva_d/100)) *(df.por_des/100)) )) - ( ((df.vtotal/(1+(df.iva_d/100))-(df.vtotal/(1+(df.iva_d/100)) *(df.por_des/100)) )) * (f.por_desc/100))) * (1+(df.iva_d/100)) AS vtotal " _
                             & " FROM  detafac" & p & " df, facturas" & p & " f, terceros t WHERE f.nitc = t.nit " _
                         & " AND (f.doc = df.doc)  " & c & " AND df.tipo_it = 'S' AND right(f.fecha,2) <= '" & txtdi2.Text & "'"
                        End If
                    End If
                Next
            ...
            Else
                ...
                For i = Val(cbini.Text) To Val(cbfin.Text)
                    ...
                    If cbini.Text = cbfin.Text Then
                        sql = sql & " SELECT  df.doc, df.nomart AS nomart, df.codart ,  df.cantidad AS item, " _
                         & " ((df.valor-(df.valor*(df.por_des/100))) - (( ((df.valor-(df.valor*(df.por_des/100))) / ( 1 + ( df.iva_d /100 ) ))* f.por_desc /100) ) - (((df.valor-(df.valor*(df.por_des/100)))-((df.valor-(df.valor*(df.por_des/100)))/(1+(df.iva_d /100))))) - ((((df.valor-(df.valor*(df.por_des/100)))/(1+(df.iva_d /100))) * f.por_ret_f/100))) As valor , " _
                             & "(( (((df.vtotal-(df.vtotal*(df.por_des/100)))-((df.vtotal-(df.vtotal*(df.por_des/100)))*(df.por_des/100))) / ( 1 + ( df.iva_d /100 ) ))* f.por_desc /100)) AS cantidad, " _
                             & " (((df.vtotal-(df.vtotal*(df.por_des/100))) - (( ((df.vtotal-(df.vtotal*(df.por_des/100))) / ( 1 + ( df.iva_d /100 ) ))* f.por_desc /100)) - (((df.vtotal-(df.vtotal*(df.por_des/100)))-((df.vtotal-(df.vtotal*(df.por_des/100)))/(1+(df.iva_d /100))))) - ((((df.vtotal-(df.vtotal*(df.por_des/100)))/(1+(df.iva_d /100))) * f.por_ret_f/100))) * df.iva_d/100) AS costo, " _
                             & " (((df.vtotal/(1+(df.iva_d/100))-(df.vtotal/(1+(df.iva_d/100)) *(df.por_des/100)) )) - ( ((df.vtotal/(1+(df.iva_d/100))-(df.vtotal/(1+(df.iva_d/100)) *(df.por_des/100)) )) * (f.por_desc/100))) * (1+(df.iva_d/100)) AS vtotal " _
                             & " FROM  detafac" & p & " df, facturas" & p & " f LEFT JOIN terceros t ON f.nitc = t.nit " _
                           & " WHERE (f.doc = df.doc) " & c & "  AND df.tipo_it = 'S' AND left(df.doc,2)<> '" & doc_aj & "' AND right(f.fecha,2) BETWEEN '" & txtdi1.Text & "' AND  '" & txtdi2.Text & "' "
                    Else
                        If p = cbini.Text Then
                            sql = sql & " SELECT  df.doc, df.nomart AS nomart, df.codart ,  df.cantidad AS item, " _
                         & " ((df.valor-(df.valor*(df.por_des/100))) - (( ((df.valor-(df.valor*(df.por_des/100))) / ( 1 + ( df.iva_d /100 ) ))* f.por_desc /100) ) - (((df.valor-(df.valor*(df.por_des/100)))-((df.valor-(df.valor*(df.por_des/100)))/(1+(df.iva_d /100))))) - ((((df.valor-(df.valor*(df.por_des/100)))/(1+(df.iva_d /100))) * f.por_ret_f/100))) As valor , " _
                             ...
                             & " (((df.vtotal/(1+(df.iva_d/100))-(df.vtotal/(1+(df.iva_d/100)) *(df.por_des/100)) )) - ( ((df.vtotal/(1+(df.iva_d/100))-(df.vtotal/(1+(df.iva_d/100)) *(df.por_des/100)) )) * (f.por_desc/100))) * (1+(df.iva_d/100)) AS vtotal " _
                             & " FROM  detafac" & p & " df, facturas" & p & " f LEFT JOIN terceros t ON f.nitc = t.nit " _
                           & " WHERE (f.doc = df.doc) " & c & "  AND df.tipo_it = 'S' AND left(df.doc,2)<> '" & doc_aj & "' AND right(f.fecha,2) >= '" & txtdi1.Text & "' "
                        ElseIf p <> cbini.Text And p <> cbfin.Text Then
                            sql = sql & " UNION  SELECT  df.doc, df.nomart AS nomart, df.codart , df.cantidad AS item,  " _
                         & " ((df.valor-(df.valor*(df.por_des/100))) - (( ((df.valor-(df.valor*(df.por_des/100))) / ( 1 + ( df.iva_d /100 ) ))* f.por_desc /100) ) - (((df.valor-(df.valor*(df.por_des/100)))-((df.valor-(df.valor*(df.por_des/100)))/(1+(df.iva_d /100))))) - ((((df.valor-(df.valor*(df.por_des/100)))/(1+(df.iva_d /100))) * f.por_ret_f/100))) As valor , " _
                             & "(( (((df.vtotal-(df.vtotal*(df.por_des/100)))-((df.vtotal-(df.vtotal*(df.por_des/100)))*(df.por_des/100))) / ( 1 + ( df.iva_d /100 ) ))* f.por_desc /100)) AS cantidad, " _
                             & " (((df.vtotal-(df.vtotal*(df.por_des/100))) - (( ((df.vtotal-(df.vtotal*(df.por_des/100))) / ( 1 + ( df.iva_d /100 ) ))* f.por_desc /100)) - (((df.vtotal-(df.vtotal*(df.por_des/100)))-((df.vtotal-(df.vtotal*(df.por_des/100)))/(1+(df.iva_d /100))))) - ((((df.vtotal-(df.vtotal*(df.por_des/100)))/(1+(df.iva_d /100))) * f.por_ret_f/100))) * df.iva_d/100) AS costo, " _
                             & " (((df.vtotal/(1+(df.iva_d/100))-(df.vtotal/(1+(df.iva_d/100)) *(df.por_des/100)) )) - ( ((df.vtotal/(1+(df.iva_d/100))-(df.vtotal/(1+(df.iva_d/100)) *(df.por_des/100)) )) * (f.por_desc/100))) * (1+(df.iva_d/100)) AS vtotal " _
                             & " FROM  detafac" & p & " df, facturas" & p & " f LEFT JOIN terceros t ON f.nitc = t.nit " _
                           & " WHERE (f.doc = df.doc) " & c & "  AND df.tipo_it = 'S' AND left(df.doc,2)<> '" & doc_aj & "'"
                        ElseIf p = cbfin.Text Then
                            sql = sql & " UNION  SELECT  df.doc, df.nomart AS nomart, df.codart , df.cantidad AS item,  " _
                         & " ((df.valor-(df.valor*(df.por_des/100))) - (( ((df.valor-(df.valor*(df.por_des/100))) / ( 1 + ( df.iva_d /100 ) ))* f.por_desc /100) ) - (((df.valor-(df.valor*(df.por_des/100)))-((df.valor-(df.valor*(df.por_des/100)))/(1+(df.iva_d /100))))) - ((((df.valor-(df.valor*(df.por_des/100)))/(1+(df.iva_d /100))) * f.por_ret_f/100))) As valor , " _
                             & "(( (((df.vtotal-(df.vtotal*(df.por_des/100)))-((df.vtotal-(df.vtotal*(df.por_des/100)))*(df.por_des/100))) / ( 1 + ( df.iva_d /100 ) ))* f.por_desc /100)) AS cantidad, " _
                             ...
                             & " (((df.vtotal/(1+(df.iva_d/100))-(df.vtotal/(1+(df.iva_d/100)) *(df.por_des/100)) )) - ( ((df.vtotal/(1+(df.iva_d/100))-(df.vtotal/(1+(df.iva_d/100)) *(df.por_des/100)) )) * (f.por_desc/100))) * (1+(df.iva_d/100)) AS vtotal " _
                             & " FROM  detafac" & p & " df, facturas" & p & " f LEFT JOIN terceros t ON f.nitc = t.nit " _
                           & " WHERE (f.doc = df.doc) " & c & "  AND df.tipo_it = 'S' AND left(df.doc,2)<> '" & doc_aj & "'  AND right(f.fecha,2) <= '" & txtdi2.Text & "'"
                        End If
                    End If
    ...
    Private Sub txttip_MouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles txttip.MouseDoubleClick
        ...
        Try
            FrmSelServicio.lbform.Text = "inf_servicios"
    ...
    Private Sub txttip_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txttip.TextChanged
        ...
        myCommand.CommandText = "select * from servicios where codser='" & txttip.Text & "'"
        myAdapter.SelectCommand = myCommand
        myAdapter.Fill(tabla)
        If tabla.Rows.Count = 0 Then
            txtnom.Text = ""
