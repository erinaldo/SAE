...
Public Class FrmSaldosInicialesCPP
    Public lon_cta As Integer
    Public TablaMes As String
    Public col, fila, FinEdit As Integer

    Private Sub FrmSaldosInicialesCPP_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        lbcta.Text = ""
        ...
        myCommand.CommandText = "SELECT doc_fp,cta_cpp FROM par_comp LIMIT 1;"
        myAdapter.SelectCommand = myCommand
        myAdapter.Fill(tabla)
        Refresh()
        items = tabla.Rows.Count
        If items = 0 Then
            MsgBox("No hay parametros de cuentas por pagar definidos, Verifique.  ", MsgBoxStyle.Information, "Verificando.  ")
            Me.Close()
            Exit Sub
        Else
            tipo.Items.Clear()
            If Trim(tabla.Rows(0).Item("doc_fp")) <> "" Then
                tipo.Items.Add(Trim(tabla.Rows(0).Item("doc_fp")))
                lbdoc.Text = Trim(tabla.Rows(0).Item("doc_fp"))
            End If
        End If
        lbcta.Text = Trim(tabla.Rows(0).Item("cta_cpp"))
        Try
            MiConexion(bda)
            myCommand.CommandText = " UPDATE ctas_x_pagar SET salmov='S'  WHERE doc IN " _
                                    & "(SELECT DISTINCT ctas_x_pagar.doc " _
                                    & " FROM documentos00 d  WHERE d.doc= ctas_x_pagar.num AND d.tipodoc=ctas_x_pagar.tipo)  "
    ...
    Public Sub cargarclientes()
        Try
            ...
            Next
            FrmSelCliente.lbform.Text = "cpp_si"
    ...
    Public Sub BuscarClientes(ByVal nit As String)
        ...
        If items = 0 Then
            ...
            If resultado = MsgBoxResult.Yes Then
                frmterceros.lbform.Text = "cpp_si"
    ...
    Private Sub grilla_CellBeginEdit(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellCancelEventArgs) Handles grilla.CellBeginEdit
        FinEdit = 1
        Try
            If grilla.Item("tipo", e.RowIndex).Value.ToString = "" Then
                grilla.Item("tipo", e.RowIndex).Value = lbdoc.Text
            End If
        Catch ex As Exception
            grilla.Item("tipo", e.RowIndex).Value = lbdoc.Text
        End Try
    ...
    Public Sub ValoresDefecto(ByVal i)
        Try
            If grilla.Item("tipo", i).Value.ToString = "" Then
                grilla.Item("tipo", i).Value = lbdoc.Text
            End If
        Catch ex As Exception
            grilla.Item("tipo", i).Value = lbdoc.Text
        End Try
    ...
    Public Sub BuscarExistente(ByVal doc As String)
        Try
            Dim tabla As New DataTable
            myCommand.CommandText = "SELECT doc FROM ctas_x_pagar WHERE doc='" & doc & "';"
            myAdapter.SelectCommand = myCommand
            myAdapter.Fill(tabla)
            Dim item As Integer = tabla.Rows.Count
            If item > 0 Then
                MsgBox("Documento " & doc & " ya existe en los registros de cuentas por pagar. ", MsgBoxStyle.Information)
            End If
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
        ...
        Else
            ...
            If tabla.Rows.Count <= 0 Then
                If nivel_cuenta(grilla.Item("cuenta", fila).Value) = True Then
                    ...
                    FrmCuentas.txtcuenta.Text = cuenta
                    FrmCuentas.lbform.Text = "cpp_si_g"
                    FrmCuentas.lbfila.Text = fila
                    FrmCuentas.ShowDialog()
                Else
                    SendKeys.Send(Chr(Keys.Tab))
    ...
    Function validar()
        If txtcliente.Text = "" Or txtnit.Text = "" Then
            MsgBox("Verifique los datos del Proveedor. ", MsgBoxStyle.Information)
            txtnit.Focus()
            Return False
            Exit Function
        End If
        ...
        For i = 0 To grilla.RowCount - 1
            Try
                If grilla.Item("cuenta", i).Value <> "" And grilla.Item("doc", i).Value <> "" And grilla.Item("concepto", i).Value <> "" And grilla.Item("fecha", i).Value.ToString <> "" Then
                    ...
                    End If
                    Try
                        If Trim(grilla.Item("doc_afe", i).Value) = "" Then
                            MsgBox("Verifique el documento externo  de la factura. ", MsgBoxStyle.Information)
                            grilla.Focus()
                            Return False
                            Exit Function
                        End If
    ...
    Public Sub EnviarGuardar()
        Try
            ...
            If sw > 0 Then
                ...
                txtcta.Enabled = False
                txtcentro.Enabled = False
                MsgBox("Se Ingresaron " & sw & " Documentos Correctamente, Verifique.  ", MsgBoxStyle.Information, "SAE Control")
            Else
                MsgBox("NO Se Ingresaron Documentos, Verifique que los datos sean correctos.  ", MsgBoxStyle.Information, "SAE Control")
            End If
        Catch ex As Exception
            ' MsgBox(ex.ToString)
        End Try
    ...
    Public Sub ActualizarConsecutivo(ByVal fila As Integer)
        Dim tabla As New DataTable
        Dim campo As String = "actualfc"
        myCommand.CommandText = "SELECT " & campo & " FROM tipdoc WHERE tipodoc='" & grilla.Item("tipo", fila).Value & "';"
        myAdapter.SelectCommand = myCommand
        myAdapter.Fill(tabla)
        Refresh()
        Try
            If Val(tabla.Rows(0).Item(campo).ToString) < Val(grilla.Item("doc", fila).Value) Then
                myCommand.CommandText = "UPDATE tipdoc SET " & campo & "=" & Val(grilla.Item("doc", fila).Value) & " WHERE tipodoc='" & grilla.Item("tipo", fila).Value & "';"
                myCommand.ExecuteNonQuery()
            End If
        Catch ex As Exception
        End Try
    End Sub
    Public Sub Guardar_CPP(ByVal fila As Integer)
        ...
        myCommand.Parameters.AddWithValue("?doc_ext", grilla.Item("doc_afe", fila).Value)
        ...
        'moneda
        myCommand.Parameters.AddWithValue("?moneda", "SI")
        ...
        myCommand.ExecuteNonQuery()
    End Sub
    Public Sub Guardar_Cta_22(ByVal fila)
        Try
            Dim fecha As Date = CDate(grilla.Item("fecha", fila).Value)
            ...
            myCommand.Parameters.AddWithValue("?modulo", "compras")
            'INSERTAR CONTABLE
    ...
    Private Sub txtcentro_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtcentro.LostFocus
        If Trim(txtncentro.Text) = "" Then
            ...
            FrmSelCentroCostos.lbform.Text = "cpp_si"
    ...
    Private Sub cmdInf_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdInf.Click
        ...
        tt = "SALDOS INICIALES DE PROVEEDORES"
        ...
        If td.Rows.Count > 0 Then
            tdoc = td.Rows(0).Item(0)
        End If

        ' DOCUMENTOS00
        Dim docu As String = ""
        Dim tdc As New DataTable
        myCommand.CommandText = "SELECT DISTINCT CAST(CONCAT(tipodoc, doc)  AS CHAR(50) ) AS d FROM documentos00 WHERE tipodoc = '" & tdoc & "' ;"
        myAdapter.SelectCommand = myCommand
        myAdapter.Fill(tdc)
        ...
        & " concat(nitc,' ',nomnit) as nomnit, total, (total-pagado) AS pagado FROM ctas_x_pagar " _
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        FrmEliminarSICart.lbform.Text = "cartPro"
