...
Imports System.IO
Public Class FrmCompEgreCpp
    Public col, fila, FinEdit As Integer
    Dim nc As String
    Private Sub FrmCompEgreCpp_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        lbestado.Text = "NULO"
    ...
    Private Sub cmdprint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdprint.Click
        ...
        Else
            ...
            resultado = MsgBox("Desea imprimir Sin formato de cheque?", MsgBoxStyle.YesNo, "Verificando")
    ...
    Private Sub CmdPrimero_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdPrimero.Click
        Try
            Bloquear()
            Dim mes As String = "ot_cpp" & PerActual(0) & PerActual(1)
        ...
        Try
            ...
            Else
                ...
                For i = 0 To tabla.Rows.Count - 1
                    ...
                    grilla.Item("DocAnti", i).Value = tabla.Rows(i).Item("doc_anti").ToString
    ...
    Public Sub BuscarCC()
        Try
            ...
            Else
                ...
                FrmSelCentroCostos.lbform.Text = "cpp_egre"
    ...
    Private Sub cmdver_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdver.Click
        ...
        Try
            FrmSelOtdoc.lbtabla.Text = "ot_cpp" & PerActual(0) & PerActual(1) & ""
            FrmSelOtdoc.tdoc.Text = td
            FrmSelOtdoc.lbform.Text = "ComEgreCpp"
    ...
    Public Sub GuardarTodo()
        ...
        While ban = 0
            tabla.Clear()
            myCommand.CommandText = "SELECT doc FROM ot_cpp" & PerActual(0) & PerActual(1) & " WHERE doc ='" & lbdoc.Text & txtnumero.Text & "';"
            myAdapter.SelectCommand = myCommand
            myAdapter.Fill(tabla)
        ...
        For i = 0 To grilla.RowCount - 1
            'If grilla.Item(0, i).Value <> "" And (grilla.Item(2, i).Value <> Moneda(0) And grilla.Item(3, i).Value <> Moneda(0)) Then
            If grilla.Item(0, i).Value <> "" Then
                If (CDbl(grilla.Item(2, i).Value) + CDbl(grilla.Item(3, i).Value)) <> Moneda(0) Then
                    Try
                        ...
                        Guardar(i)
                    Catch ex As Exception
                        GuardarError(ex.ToString, "COMP_EGRE_CPP", "Guardar")
                        MsgBox(ex.ToString)
                        Exit Sub
                    End Try
                End If
        ...
        If FrmPrincipal.cmdAuditoria.Visible = True Then
            Guar_MovUser("COMPRAS", "GUARDAR COMPROBANTE DE CXP Nº: " & txttipo.Text & txtnumero.Text, "", "", "")
        End If
        '.....
    ...
    Public Sub ModificarTodo()
        ...
        If resultado = MsgBoxResult.Yes Then
            ...
            For i = 0 To grilla.RowCount - 1
                If grilla.Item(0, i).Value <> "" Then
                    If (CDbl(grilla.Item(2, i).Value) + CDbl(grilla.Item(3, i).Value)) <> Moneda(0) Then
                        ...
                        Catch ex As Exception
                            MsgBox(ex.ToString)
                            GuardarError(ex.ToString, "COMP_EGRE_CPP", "Modificar")
                            Exit Sub
                        End Try
                    End If
                End If
            ...
            '.....
            If FrmPrincipal.cmdAuditoria.Visible = True Then
                Guar_MovUser("COMPRAS", "MODIFICAR COMPROBANTE DE CXP Nº: " & txttipo.Text & txtnumero.Text, "", "", "")
            End If
            '.....
            MsgBox("La Base De Datos Se Actualizó Correctamente.  ", MsgBoxStyle.Information, "Guardar Datos")
        End If
    ...
    Public Sub Pagos(ByVal fila As Integer)
        ...
        Try
            If CDbl(grilla.Item("abonado", fila).Value) > 0 Then
                ...
                ct = ct + 1
                myCommand.Parameters.Clear()
                myCommand.Parameters.AddWithValue("?abonado", DIN(Moneda(grilla.Item("abonado", fila).Value)).ToString)
                myCommand.CommandText = "UPDATE ctas_x_pagar SET pagado=pagado+?abonado WHERE doc=TRIM('" & grilla.Item("doc_afe", fila).Value.ToString & "');"
                myCommand.ExecuteNonQuery()
            End If
        Catch ex As Exception
            GuardarError(ex.ToString, "COMP_EGRE_CPP", "GuardarPago")
            If ct = 1 Then
                Pagos(fila)
            End If
        End Try
    ...
    Function ValidarGuardar()
        ...
        If Trim(txttipo.Text) = "" Then
            MsgBox("No ha seleccionado el tipo de documento, Verifique.  ", MsgBoxStyle.Information, "SAE Control ")
            txttipo.Focus()
            Return False
            Exit Function
        ElseIf txtcentro.Enabled = True And txtncentro.Text = "" Then
            MsgBox("No ha digitado centro de costos, Verifique.  ", MsgBoxStyle.Information, "SAE Control ")
            txtncentro.Focus()
            Return False
            Exit Function
        ElseIf txttipo.Text <> nc And txtvalor.Text = "0,00" Then
    ...
    Public Sub Guardar(ByVal fila As Integer)
        ...
        ElseIf lbfp.Text = "Consignacion No." Or lbfp.Text = "consignacion" Or lbfp.Text = "consignacion" Then
        ...
        Try
            myCommand.Parameters.AddWithValue("?doc_anti", grilla.Item("DocAnti", fila).Value.ToString)
        Catch ex As Exception
    ...
    Public Sub GuardarAnticipos(ByVal fila As Integer)
        Dim Sql As String = ""
        Try
            If Trim(grilla.Item("DocAnti", fila).Value) <> "" Then
                myCommand.Parameters.Clear()
                Sql = "UPDATE ant_a_prov SET causado = causado + " & DIN(Moneda(grilla.Item("Creditos", fila).Value)) & " WHERE per_doc='" & Trim(grilla.Item("DocAnti", fila).Value) & "';"
                'MsgBox(Sql)
    ...
    Public Sub GuardarContable(ByVal fila As Integer)
        ...
        Try
            ...
            myCommand.Parameters.Clear()
        Catch ex As Exception
            GuardarError(ex.ToString, "COMP_EGRE_CPP", "GuardarContable")
            If ct = 1 Then
                GuardarContable(fila)
            End If
        End Try
    End Sub
    ...
    Private Sub GenerarPDFCheque()
        ...
        Try
            ...
            Observacion()
            k = k - 20
            cb.ShowTextAligned(50, "Impreso a la fecha y hora: " & Now & " por el usuario: " & FrmPrincipal.lbuser.Text & ". Modulo:Cuentas por Pagar", 10, k, 0)
            k = k - 10
        ...
        Try
            ...
            Else
                ...
                If tabla.Rows(0).Item("aj") = lbdoc.Text Then
                    tabla.Clear()
                    myCommand.CommandText = "SELECT doc_afec FROM ot_cpp" & PerActual(0) & PerActual(1) & " WHERE doc='" & lbdoc.Text.ToString & txtnumero.Text.ToString & "';"
                    myAdapter.SelectCommand = myCommand
                    myAdapter.Fill(tabla)
        ...
        Try
            ...
            ElseIf i = 1 Then
                If lbfp.Text = "Tarjeta" Then
                    img1 = iTextSharp.text.Image.GetInstance(My.Application.Info.DirectoryPath & "\Reportes\CE\CE1_2.jpg")
                ElseIf lbfp.Text = "Consignacion" Or lbfp.Text = "consignacion" Or lbfp.Text = "Consignacion No." Then
                    img1 = iTextSharp.text.Image.GetInstance(My.Application.Info.DirectoryPath & "\Reportes\CE\CE1_C.jpg")
                Else
    ...
    Public Sub ColocarImg(ByVal i As Integer)
        Try
            ...
            If i = 1 Then
                If lbfp.Text = "Tarjeta" Then
                    img = iTextSharp.text.Image.GetInstance(My.Application.Info.DirectoryPath & "\Reportes\CE\CE1_2.jpg")
                ElseIf lbfp.Text = "Consignacion" Or lbfp.Text = "consignacion" Or lbfp.Text = "Consignacion No." Then
                    img = iTextSharp.text.Image.GetInstance(My.Application.Info.DirectoryPath & "\Reportes\CE\CE1_C.jpg")
                Else
    ...
    Private Sub cmditems_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmditems.Click
        ...
        If lbestado.Text = "EDITAR" Then
            FrmItemsCPP.txttipo.Enabled = False
        Else
            FrmItemsCPP.txttipo.Enabled = True
        End If
        ...
        End If
        FrmItemsCPP.txtnitc.Text = txtnit.Text
        FrmItemsCPP.txtvalor.Text = txtvalor.Text
        FrmItemsCPP.txtcliente.Text = lbcliente.Text
        FrmItemsCPP.txttipo.Text = txttipo.Text
        FrmItemsCPP.txtperiodo.Text = "/" & PerActual
        FrmItemsCPP.txtnumfac.Text = NumeroDoc(txtnumero.Text)
        FrmItemsCPP.lbper.Text = PerActual
        Try
            FrmItemsCPP.txtdia.Text = txtdia.Text
            Dim f As Date = CDate(FrmItemsCPP.txtdia.Text & FrmItemsCPP.txtperiodo.Text)
        Catch ex As Exception
            FrmItemsCPP.txtdia.Text = "01"
        End Try
        '*********************************************
        FrmItemsCPP.txttipo.Text = lbdoc.Text
        FrmItemsCPP.ShowDialog()
        CalularTotales()
    End Sub
    ...
    Public Sub Desengloblar(ByVal f As Integer)

    End Sub

    Private Sub grilla_CellMouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles grilla.CellMouseDoubleClick
        Try
            If e.ColumnIndex = 0 And grilla.Item(0, e.RowIndex).Value.ToString <> "" Then
                Dim resultado As MsgBoxResult
                resultado = MsgBox("La cuenta " & grilla.Item(0, fila).Value & " se va a Desenglobar, ¿Desea realizar este proceso?", MsgBoxStyle.YesNo, "Verificando")
                If resultado = MsgBoxResult.Yes Then
                    Try
                        FrmDesenglobarCta.lbform.Text = "egrCpp"
                        FrmDesenglobarCta.lbfila.Text = fila
                        FrmDesenglobarCta.lbcta.Text = grilla.Item(0, fila).Value.ToString
                        FrmDesenglobarCta.lbnombre.Text = grilla.Item(1, fila).Value.ToString
                        FrmDesenglobarCta.lbdb.Text = grilla.Item("Debitos", fila).Value.ToString
                        FrmDesenglobarCta.lbcr.Text = grilla.Item("Creditos", fila).Value.ToString
                        FrmDesenglobarCta.lbdoc.Text = txttipo.Text
                        FrmDesenglobarCta.lbnomdoc.Text = lbnomdoc.Text
                        FrmDesenglobarCta.lbnum.Text = txtnumero.Text
                        FrmDesenglobarCta.lbper.Text = PerActual
                        FrmDesenglobarCta.ShowDialog()
                    Catch ex As Exception
