Public Class FrmSelMetas
    Public fila As Integer

    Private Sub FrmSelMetas_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            gitems.Rows.Clear()
            Dim tabla As New DataTable
            tabla.Clear()
            Dim items As Integer
            myCommand.CommandText = "SELECT codigo, area, tope FROM metas_x_area ;"
            myAdapter.SelectCommand = myCommand
            myAdapter.Fill(tabla)
            Refresh()
            items = tabla.Rows.Count
            If items = 0 Then
                MsgBox("No existen metas, Verifique.  ", MsgBoxStyle.Information, "Informacion ")
                Exit Sub
            End If
            Try
                gitems.Rows.Clear()
            ...
            For i = 0 To items - 1
                gitems.Item(0, i).Value = tabla.Rows(i).Item("codigo")
                gitems.Item(1, i).Value = tabla.Rows(i).Item("area")
                gitems.Item(2, i).Value = Moneda(tabla.Rows(i).Item("tope"))
            Next
            With gitems
    ...
    Public Sub seleccionar(ByVal mifila As Integer)
        ...
        Select Case lbform.Text
            Case "metas"
                FrmMetasxArea.txtcod.Text = gitems.Item(0, mifila).Value
    ...
    Public Sub BuscarGrilla(ByVal cad As String)
        ...
        Try
            If cmbbuscar.Text = "CODIGO" Then
                cl = 0
            ElseIf cmbbuscar.Text = "AREA" Then
                cl = 1
            End If
