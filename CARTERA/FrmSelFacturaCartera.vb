Public Class FrmSelFacturaCartera
    Public fila As Integer
    Private Sub FrmSelFacturaCartera_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        BuscarPeriodo()
        Dim tabla As New DataTable
        tabla.Clear()
        Dim items As Integer
        myCommand.CommandText = "SELECT doc, fecha, nomnit, total, pagado, (total - pagado) AS saldo, tipo, num FROM cobdpen WHERE nitc='" & FrmConUnaFC.txtnit.Text & "' AND tipo='" & FrmConUnaFC.txttipo.Text & "' ORDER BY doc,fecha;"
        ...
        For i = 0 To items - 1
            ...
            gitems.Item(3, i).Value = tabla.Rows(i).Item("nomnit")
    ...
    Public Sub seleccionar(ByVal mifila As Integer)
        ...
        FrmConUnaFC.lbsel.Text = "si"
        FrmConUnaFC.txttipo.Text = gitems.Item("tipo", mifila).Value()
        FrmConUnaFC.txtnumfac.Text = gitems.Item("numero", mifila).Value()
        gitems.Focus()
        Me.Close()
    End Sub
