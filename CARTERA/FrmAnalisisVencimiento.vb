...
Public Class FrmAnalisisVencimiento


    Private Sub FrmAnalisisVencimiento_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Public Sub cargarclientes()
        Try
            ...
            Next
            FrmSelCliente.lbform.Text = "inf_car_ana_ven"
    ...
    Private Sub cmdpantalla_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdpantalla.Click
        ...
        If checkresumido.Checked = False Then
            sql = " SELECT c.nitc, c.nomnit, t.telefono AS nitcod, c.doc, CAST( c.fecha AS CHAR( 20 ) ) AS fecha, " _
             ...
             & " WHERE c.nitc=t.nit AND c.total > c.pagado  " & ff & " " & cl & " " & dc & ""
        ...
        Else
            ...
            sql = " SELECT  c.nomnit, " _
            & " SUM(IF ( (SELECT DATEDIFF( NOW( ) ,  (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) ) ) < " & r1 & ", (c.total - c.pagado), 0) )AS retiva,  " _
            & " SUM(IF ((SELECT DATEDIFF( NOW( ) ,  (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) ) ) > " & r11 & " AND (SELECT DATEDIFF( NOW( ) ,  (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) ) ) < " & r2 & " , (c.total - c.pagado), 0 )) AS retica,  " _
            & " SUM(IF ( (SELECT DATEDIFF( NOW( ) ,  (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) ) ) > " & r22 & " AND (SELECT DATEDIFF( NOW( ) ,  (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) ) ) < " & r3 & " , (c.total - c.pagado), 0 )) AS pagado,  " _
            & " SUM(IF ( (SELECT DATEDIFF( NOW( ) ,  (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) ) ) > " & r33 & " AND (SELECT DATEDIFF( NOW( ) ,  (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) ) ) < " & r4 & " , (c.total - c.pagado), 0 )) AS vpos,  " _
            & " SUM(IF ( (SELECT DATEDIFF( NOW( ) ,  (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) ) ) > " & r5 & ", (c.total - c.pagado), 0 )) AS tasa,  " _
            & " SUM(total - pagado) AS total FROM cobdpen c, terceros t  WHERE c.nitc=t.nit AND c.total > c.pagado " & ff & " " & cl & " " & dc & " AND left(c.doc,2)<>'" & doc_aj & "'  GROUP BY c.nitc ORDER BY nomnit"
            ...
            CrReport.Load(My.Application.Info.DirectoryPath & "\Reportes\ReportAnVeD.rpt")
            CrReport.SetDataSource(tabla)
            CrReport.PrintOptions.PaperSize = PaperSize.PaperA4
            FrmReportAnaVen.CrystalReportViewer1.ReportSource = CrReport
            ...
            Try
                ...
                FrmReportAnaVen.CrystalReportViewer1.ParameterFieldInfo = prmdatos
                FrmReportAnaVen.ShowDialog()
