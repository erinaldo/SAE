...
Public Class FrmCarxCiu
    ...
    Private Sub pdf()
        ...
        End If
        If f1.Checked = True Then ' FECHA FACTURA
            ff = " AND c.fecha >= '" & fc & "'"
        Else
            ff = " AND (SELECT ADDDATE(  c.fecha, INTERVAL c.vmto DAY) ) >= '" & fc & "' AND (SELECT ADDDATE( c.fecha, INTERVAL c.vmto DAY) ) <= NOW() "
        End If
        If d2.Checked = True Then ' Una ciudad
            ciu = " AND m.descripcion LIKE  '" & Txtconp.Text & "%' "
        End If
        ...
        If checkresumido.Checked = False Then
            ...
            sql = " Select t.mun, m.descripcion AS persona, c.nitc as nit, c.nomnit as nombre, c.doc as mes, CAST( c.fecha AS CHAR( 20 ) ) AS dir, " _
          & " CAST( (SELECT ADDDATE( c.fecha, INTERVAL c.vmto DAY) ) AS CHAR( 20 ) ) as tipo, " _
          & " IF ( (SELECT DATEDIFF( NOW( ) , (SELECT ADDDATE( c.fecha, INTERVAL c.vmto DAY) ) ) ) < " & r1 & ", (c.total - c.pagado), 0) AS reteiva, " _
          & " IF ((SELECT DATEDIFF( NOW( ) , (SELECT ADDDATE( c.fecha, INTERVAL c.vmto DAY) ) ) ) > " & r11 & " AND (SELECT DATEDIFF( NOW( ) , (SELECT ADDDATE( c.fecha, INTERVAL c.vmto DAY) ) ) ) < " & r2 & " , (c.total - c.pagado), 0 ) AS reteica, " _
          & " IF ( (SELECT DATEDIFF( NOW( ) , (SELECT ADDDATE( c.fecha, INTERVAL c.vmto DAY) ) ) ) > " & r22 & " AND (SELECT DATEDIFF( NOW( ) , (SELECT ADDDATE( c.fecha, INTERVAL c.vmto DAY) ) ) ) < " & r3 & " , (c.total - c.pagado), 0 ) AS pais, " _
          & " IF ( (SELECT DATEDIFF( NOW( ) , (SELECT ADDDATE( c.fecha, INTERVAL c.vmto DAY) ) ) ) > " & r33 & " AND (SELECT DATEDIFF( NOW( ) , (SELECT ADDDATE( c.fecha, INTERVAL c.vmto DAY) ) ) ) < " & r4 & " , (c.total - c.pagado), 0 ) AS dept, " _
          & " IF ( (SELECT DATEDIFF( NOW( ) , (SELECT ADDDATE( c.fecha, INTERVAL c.vmto DAY) ) ) ) > " & r5 & ", (c.total - c.pagado), 0 ) AS cupo, (total - pagado) AS vmto FROM cobdpen c, terceros t, sae.mun m " _
          & " WHERE c.nitc=t.nit AND t.mun = m.codmun AND c.total > c.pagado " & ff & "  " & ciu & "  " & cl & " ORDER BY m.descripcion, c.nitc, c.doc "
            ...
            CrReport.Load(My.Application.Info.DirectoryPath & "\Reportes\ReportCarxCiu.rpt")
            CrReport.SetDataSource(tabla)
            CrReport.PrintOptions.PaperSize = PaperSize.PaperA4
            FrmReportCarxCiu.CrystalReportViewer1.ReportSource = CrReport
            ...
            Try
                ...
                Dim Prrr As New ParameterField
                ...
                Prrr.Name = "re"
                Prrr.CurrentValues.AddValue(rr.ToString)
                ...
                FrmReportCarxCiu.CrystalReportViewer1.ParameterFieldInfo = prmdatos
                FrmReportCarxCiu.ShowDialog()
        ...
        Else
            ...
            sql = " SELECT t.mun, m.descripcion AS persona, c.nitc as nit, c.nomnit as nombre, " _
            & " SUM(IF ( (SELECT DATEDIFF( NOW( ) , (SELECT ADDDATE( c.fecha, INTERVAL c.vmto DAY) ) ) ) < " & r1 & ", (c.total - c.pagado), 0)) AS reteiva, " _
            & " SUM(IF ((SELECT DATEDIFF( NOW( ) , (SELECT ADDDATE( c.fecha, INTERVAL c.vmto DAY) ) ) ) > " & r11 & " AND (SELECT DATEDIFF( NOW( ) , (SELECT ADDDATE( c.fecha, INTERVAL c.vmto DAY) ) ) ) < " & r2 & " , (c.total - c.pagado), 0 )) AS reteica, " _
            & " SUM(IF ( (SELECT DATEDIFF( NOW( ) , (SELECT ADDDATE( c.fecha, INTERVAL c.vmto DAY) ) ) ) > " & r22 & " AND (SELECT DATEDIFF( NOW( ) , (SELECT ADDDATE( c.fecha, INTERVAL c.vmto DAY) ) ) ) < " & r3 & " , (c.total - c.pagado), 0 )) AS pais, " _
            & " SUM(IF ( (SELECT DATEDIFF( NOW( ) , (SELECT ADDDATE( c.fecha, INTERVAL c.vmto DAY) ) ) ) > " & r33 & " AND (SELECT DATEDIFF( NOW( ) , (SELECT ADDDATE( c.fecha, INTERVAL c.vmto DAY) ) ) ) < " & r4 & " , (c.total - c.pagado), 0 )) AS dept, " _
            & " SUM(IF ( (SELECT DATEDIFF( NOW( ) , (SELECT ADDDATE( c.fecha, INTERVAL c.vmto DAY) ) ) ) > " & r5 & ", (c.total - c.pagado), 0 )) AS cupo, SUM(total - pagado) AS vmto FROM cobdpen c, terceros t, sae.mun m " _
            & " WHERE c.nitc=t.nit AND t.mun = m.codmun AND c.total > c.pagado AND left(c.doc,2)<>'" & doc_aj & "'  " & ff & "  " & ciu & "  " & cl & " GROUP BY nitc ORDER BY descripcion"
            ...
            CrReport.Load(My.Application.Info.DirectoryPath & "\Reportes\ReportCarxCiuD.rpt")
            CrReport.SetDataSource(tabla)
            CrReport.PrintOptions.PaperSize = PaperSize.PaperA4
            FrmReportCarxCiuD.CrystalReportViewer1.ReportSource = CrReport
            ...
            Try
                ...
                FrmReportCarxCiuD.CrystalReportViewer1.ParameterFieldInfo = prmdatos
                FrmReportCarxCiuD.ShowDialog()
    ...
    Private Sub FrmCarxCiu_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Public Sub cargarclientes()
        Try
            ...
            Next
            FrmSelCliente.lbform.Text = "lista_clie_carXciu"
