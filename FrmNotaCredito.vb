...
Public Class FrmNotaCredito
    ...
    Public Sub BuscarActual()
        If lbestado.Text = "NUEVO" Then
            ...
            If tabla.Rows.Count < 1 Then
                MsgBox("Favor no han creado tipo de documento para  las Notas Crédito (del grupo NC), verifique.   ", MsgBoxStyle.Information, "Verificando")
                Me.Close()
                Exit Sub
            End If
    ...
    Private Sub cmdver_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdver.Click
        Try
            ...
            FrmSelOtdoc.lbform.Text = "NotaCr"
                    ...
                    If lbestado.Text = "NUEVO" Then
                        ...
                        For i = 0 To grilla.RowCount - 1
                            If grilla.Item(0, i).Value <> "" And Moneda(grilla.Item(1, i).Value) <> "0,00" Then
                                Try
                                    it = it + 1
                                    Guardar(it, txtcuenta.Text, 0, grilla.Item(1, i).Value, grilla.Item(0, i).Value)
                                    GuardarContable(it, txtcuenta.Text, 0, grilla.Item(1, i).Value, grilla.Item(0, i).Value)
                                    ActualizarCuentas(txtcuenta.Text, 0, grilla.Item(1, i).Value)
                                Catch ex As Exception
                                    MsgBox(ex.ToString)
                                End Try
                            End If
                        ...
                        For i = 0 To grilla2.RowCount - 1
                            If grilla2.Item(0, i).Value <> "" And Moneda(grilla2.Item(1, i).Value) <> "0,00" Then
                                Try
                                    it = it + 1
                                    Guardar(it, grilla2.Item(0, i).Value, grilla2.Item(1, i).Value, 0, "DEBITOS")
                                    GuardarContable(it, grilla2.Item(0, i).Value, grilla2.Item(1, i).Value, 0, "DEBITOS")
                                    ActualizarCuentas(grilla2.Item(0, i).Value, grilla2.Item(1, i).Value, 0)
                                Catch ex As Exception
                                    MsgBox(ex.ToString)
                                End Try
                            End If
    ...
    Private Sub CmdPrimero_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdPrimero.Click
        Try
            ...
            Dim tabla As New DataTable
            myCommand.CommandText = "SELECT DISTINCT(doc) FROM " & mes & " WHERE grupo='NC' ORDER BY doc, item LIMIT 0, 1;"
            myAdapter.SelectCommand = myCommand
            myAdapter.Fill(tabla)
    ...
    Private Sub CmdAtras_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdAtras.Click
        Try
            ...
            If i > 0 Then
                i = i - 1
                Dim tabla As New DataTable
                myCommand.CommandText = "SELECT DISTINCT(doc) FROM " & mes & " WHERE grupo='NC' ORDER BY doc, item LIMIT " & i & ", 1;"
                myAdapter.SelectCommand = myCommand
                myAdapter.Fill(tabla)
                Refresh()
                BuscarDocumento(tabla.Rows(0).Item(0))
    ...
    Private Sub CmdSiguiente_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdSiguiente.Click
        Try
            ...
            myCommand.CommandText = "SELECT DISTINCT(doc) FROM " & mes & " WHERE grupo='NC';"
            myAdapter.SelectCommand = myCommand
            myAdapter.Fill(tabla2)
            Refresh()
    ...
    Public Sub BuscarCC()
        ...
        Try
            ...
            Else
                ...
                FrmSelCentroCostos.lbform.Text = "nc"
    ...
    Public Sub Guardar(ByVal it As Integer, ByVal codigo As String, ByVal db As Double, ByVal cr As Double, ByVal des As String)
        ...
        myCommand.Parameters.AddWithValue("?grupo", "NC")
        myCommand.Parameters.AddWithValue("?tipodoc", lbdoc.Text)
    ...
    Public Sub ColocarImg(ByVal i As Integer)
        Try
            ...
            If i = 1 Then
                img = iTextSharp.text.Image.GetInstance(My.Application.Info.DirectoryPath & "\Reportes\NOTA\NC.jpg")
    ...
    Private Sub cmdmas_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdmas.Click
        'If lbestado.Text <> "NUEVO" And lbestado.Text <> "EDITAR" Then Exit Sub
        'SendKeys.Send(Chr(Keys.Space))
        'FrmSelConcepImp.lbform.Text = "NotaCr"
