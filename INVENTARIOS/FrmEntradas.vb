...
Public Class FrmEntradas
    ...
    Private Sub cmdNuevo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdNuevo.Click
        ...
        Else
            ...
            Catch ex As Exception
                txtnumfac.Text = NumeroDoc(1)
            End Try
            cbtipo.Text = "ENTRADA DE MERCANCIA"
            lbestado.Text = "NUEVO"
    ...
    Private Sub CmdMostrar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdMostrar.Click
        FrmSelMovInventario.lbform.Text = "entradas"
    ...
    Public Sub cargarclientes()
        Try
            FrmSelCliente.lbform.Text = "entradas"
    ...
    Public Sub BuscarClientes(ByVal nit As String)
        ...
        If items = 0 Then
            ...
            If resultado = MsgBoxResult.Yes Then
                ...
                frmterceros.lbform.Text = "entradas"
    ...
    Public Sub GuardarFactura()
        Try
            ...
            myCommand.Parameters.AddWithValue("?o_compra", txtcompra.Text.ToString)
            ...
            '.....
            If FrmPrincipal.cmdAuditoria.Visible = True Then
                Guar_MovUser("INVENTARIO", "GUARDAR ENTRADA Nº: " & txttipo.Text & txtnumfac.Text, "", "", "")
            End If
            '.....
    ...
    Public Sub Actualizar_Con_inv(ByVal fila As Integer, ByVal f As String, ByVal margen As Integer)
        Try
            ...
            End Try
            '************************************************
            myCommand.CommandText = "SELECT costprom,margen,precio1 FROM con_inv WHERE codart='" & gfactura.Item(1, fila).Value & "' AND periodo<='" & PerActual(0) & PerActual(1) & "';"
            myAdapter.SelectCommand = myCommand
            myAdapter.Fill(tabla)
            suma = 0
    ...
    Public Sub GuardarContable()
        Try
            ...
            If tabla.Rows(0).Item("contable").ToString <> "no" Then 'HAY INTERFAZ CONTABLE
                ...
                For i = 0 To gfactura.RowCount - 1
                    MovimientoContable(i, "inv", gfactura.Item("ctainv", i).Value, "ENTRADA A BOD. " & cbdestino.Text & " " & Trim(txtcliente.Text))
    ...
    Public Sub Banner()
        ...
        cb.ShowTextAligned(50, "DOCUMENTO DE ENTRADA: " & tabla.Rows(0).Item("doc") & " DEL PERIODO " & PerActual, 20, 790, 0)
        cb.ShowTextAligned(50, "FECHA ELABORACION:" & tabla.Rows(0).Item("dia") & "/" & PerActual & " " & tabla.Rows(0).Item("hora").ToString, 20, 780, 0)
        cb.ShowTextAligned(50, "BODEGA DE ENTRADA: " & cbdestino.Text & " " & txtnombod.Text, 20, 770, 0)
        cb.ShowTextAligned(50, "TIPO DE ENTRADA: " & tabla.Rows(0).Item("desc_mov"), 20, 760, 0)
        cb.ShowTextAligned(50, "NIT / C.C. " & tabla.Rows(0).Item("nitc") & "  " & Trim(txtcliente.Text), 20, 750, 0)
        cb.ShowTextAligned(50, "ORDEN DE COMPRA # " & tabla.Rows(0).Item("o_compra"), 20, 740, 0)
    ...
    Private Sub llenar_AD()
        ...
        txtcompra1.Text = txtcompra.Text
    ...
    Private Sub AuditarDatos()
        Try
            ...
            If cbdestino.Text <> cbdestino1.Text Then
                ...
                ant = ant & cbdestino1.Text & "; "
                nue = nue & cbdestino.Text & "; "
            End If
            ...
            End If
            If txtcompra.Text <> txtcompra1.Text Then
                camp = camp & " ORDEN DE COMPRA; "
                ant = ant & txtcompra1.Text & "; "
                nue = nue & txtcompra.Text & "; "
            End If
            ...
            For i = 0 To gfactura2.RowCount - 1
                ...
                Next
            Next
            Guar_MovUser("INVENTARIO", "MODIFICAR ENTRADA Nº: " & txttipo.Text & txtnumfac.Text, camp, ant, nue)
